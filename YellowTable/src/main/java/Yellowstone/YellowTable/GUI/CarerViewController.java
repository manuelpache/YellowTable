...
/**
 * Importiert die Logikklassen sowie zentrale Elemente des Java-FXML-Pakets.
...
/**
 * Der Controller des Carer-Views. Fuer naehere Information siehe Architektur.
 * 
 * @author tomlewandowski, manuelpache, Erik
 *
 */
public class CarerViewController {
	...
	/**
	 * Initialisiert die Klasse. Mit Manuels Agenda.
	 */
	 ...
	 * @param selClass
	 */
	public void loadAppointments(EducEmployee ee) {
		if (ee != null && GUIHandler.getWeekTable(ee) != null) {
			for (Unit u : GUIHandler.getWeekTable(ee)) {
	...
	/**
	 * Lädt alle BreakUnits einer Break in die Agenda
	 * 
	 * @param b
	 */
	...
	/**
	 * Methode, die den Button, um zu Drucken, realisiert.
	 * 
	 ...
	 */
	@FXML
	public void btnPrint() throws COSVisitorException, IOException,
			PrinterException {
		print();
	}
	...
	public void bPdf() throws IOException, COSVisitorException {
		...
		createPDFFromImage("rsc.pdf", node.getId() + (".png"), dir
				+ File.separator + "PaedMitarbeiterPlan "+ ee.getShortName() + (".pdf"), false);
	...
	private void bCsv() throws IOException {
		...
		// Units je nach Wochentag in ArrayListen speichern.
		...
		for (ArrayList<Unit> a : exportList) {
			...
			for (Unit u : a) {
				...
				// Wenn es sich bei dem Unit um Unterricht handelt.
				if (u instanceof TeachingUnit) {
				...
				// Wenn es sich bei dem Unit um eine Draußeneinheit handelt.
				if (u instanceof ExternUnit) {
					Set<Class> classes = new HashSet<Class>();
				...
				// Wenn es sich bei dem Unit um ein Meeting handelt.
				if (u instanceof MeetingUnit) {
					ArrayList<Room> rooms = new ArrayList<Room>();
	...
	private void bTxt() throws IOException {
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(dir
				+ File.separator + "PaedMitarbeiterPlan "+ee.getShortName()+".txt"));
		out.newLine();
		ArrayList<Unit> ultimate = new ArrayList<Unit>();
		ultimate.addAll(GUIHandler.getWeekTable(ee));
		ArrayList<ArrayList<Unit>> exportList = new ArrayList<ArrayList<Unit>>();
	...
	public void print() throws IOException, PrinterException,
		...
		document.close();
		createPDFFromImage("rsc.pdf", node.getId() + (".png"), dir
				+ File.separator + "PaedMitarbeiterPlan" + (".pdf"), true);
		File f1 = new File(node.getId() + (".png"));
		File f2 = new File("rsc.pdf");
		f1.delete();
		f2.delete();
		...
		try {
			doc2 = PDDocument.load(dir + File.separator + "PaedMitarbeiterPlan"
					+ (".pdf"));
			doc2.print();
		} catch (Exception ec) {
			ec.printStackTrace();
